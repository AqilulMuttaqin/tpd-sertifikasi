...
@section('content')
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('kategori.show', $kategori->id) }}"
                        class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2">Edit
                        Data</a>
                </div>
            </li>
        </ol>
    ...
    <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
        <h2 class="text-lg font-semibold">Edit Kategori Surat</h2>
        <p class="mt-2 text-sm text-gray-600">Edit data kategori surat. Jika sudah selesai, klik tombol "Simpan".</p>
        ...
        @endif
        <form class="space-y-4" action="{{ route('kategori.update', $kategori->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <label for="id" class="md:col-span-1 flex items-center text-sm font-medium text-gray-900">
                    ID Kategori
                </label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="md:col-span-3">
                    <textarea rows="4" id="keterangan" name="keterangan"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-4 py-2.5"
                        placeholder="Masukkan keterangan...." required>{{ $kategori->keterangan }}</textarea>
                </div>
            </div>
